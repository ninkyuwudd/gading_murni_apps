...
import {useMasterMachines} from '../api/hooks/useMasterMachines';
...
const FormMasterMachinesSelector: React.FC<IProps> = ({
  name,
  label,
  editable = true,
}) => {
  ...
  const [field, meta, helpers] = useField<MasterMachine>(name);
  const {value} = field;
  const actionSheetRef = useRef<ActionSheetRef>(null);
  const {t} = useTranslation();
  const error = meta.error as unknown as MasterMachine;
  ...
  return (
    <View style={styles.container}>
      ...
      {error && <Text style={styles.errorText}>{error.name}</Text>}
      <ActionSheet ref={actionSheetRef} containerStyle={styles.actionContainer}>
        <View style={styles.devider} />
        <Text style={styles.title}>{t('addMachineFieldFields.name')}</Text>
        <SearchBar
...
export default FormMasterMachinesSelector;
